...
class Comments {
    ...
    async setComment(id) {
        const { data } = await axios.get(`${HOST}/comment/${id}`);
        if (Object.keys(data).includes('status')) {
            ...
            for (let el of commentList) {
                this.comments.push({
                    ...
                    countReplys: (await axios.get(`${HOST}/reply/count/${el.id}`)).data
                });
            }
        }
    }
    async createComment(postId, content) {
        await axios.post(`${HOST}/comment/createComment`, { postId, content }, {
...
`;
    ...
    async modifyComment(commentId, content) {
        await axios.put(`${HOST}/comment/${commentId}`, { content }, {
            withCredentials: true
        });
    }
...
const commentRender = async () => {
    ...
    commentLikeBtn.forEach((el) => {
        el.onclick = async (e) => {
            ...
            await axios.post(`${HOST}/comment-likes/like/${comment.dataset.id}`, {}, {
                withCredentials: true
            });
        };
    });
    commentDisLikeBtn.forEach((el) => {
        el.onclick = async (e) => {
            ...
            await axios.post(`${HOST}/comment-likes/dislike/${comment.dataset.id}`, {}, {
                withCredentials: true
            });
        };
    });
