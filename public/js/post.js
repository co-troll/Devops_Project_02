...
const postMenuRender = async () => {
    ...
    postLikeBtns.forEach((el) => {
        el.onclick = async (e) => {
            ...
            if (!TOKEN)
                return;
            const btn = e.target;
            const postBox = btn.closest(".postBox");
            if (!btn.classList.contains("selected")) {
                btn.classList.add("selected");
                (_a = btn.nextElementSibling) === null || _a === void 0 ? void 0 : _a.classList.remove("selected");
    ...
    postDisLikeBtns.forEach((el) => {
        el.onclick = async (e) => {
            ...
            if (!btn.classList.contains("selected")) {
                btn.classList.add("selected");
                (_a = btn.previousElementSibling) === null || _a === void 0 ? void 0 : _a.classList.remove("selected");
